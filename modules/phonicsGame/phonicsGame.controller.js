...
import phonicsGameModel from "./phonicsGame.model.js";
...
export const getPhonicsDataByLevel = async (req, res) => {
  ...
  try {
    ...
    const phonicsGames = await phonicsGameService.getPhonicsGameByLevel(level);
    if (phonicsGames.length === 0) {
      return res.status(404).json({
        message: `No phonics games found for level ${level}`,
      });
    }
    const formattedGames = phonicsGames.map(game => ({
      ...game._doc,
...
export const deleteWord = async (req, res) => {
  try {
    ...
    const game = await phonicsGameModel.findById(id);
    if (!game) {
    ...
    if (game.words.length === 0) {
      await phonicsGameModel.deleteOne({ _id: id });
...
export const deleteLevel = async (req, res) => {
  try {
    const { id } = req.params;
    const result = await phonicsGameModel.deleteOne({ _id: id });
...
export const getLevels = async (req, res) => {
  try {
    const levels = await phonicsGameModel.distinct('level');
    res.status(200).json({
...
export const updateLevel = async (req, res) => {
  try {
    ...
    const deleteResult = await phonicsGameModel.deleteMany({ level: parsedLevel });
    console.log(`Deleted ${deleteResult.deletedCount} document(s) for level ${parsedLevel}`);

    // Create new document
    const newGame = new phonicsGameModel({
      words: validWords,
...
export const getDataGame = async (req, res) => {
  console.log("getPhonicsDataGame controller");
  ...
  try {
    ...
    const phonicsGames = await phonicsGameService.getPhonicsGame(
      level
    );
